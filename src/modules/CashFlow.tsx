...
export const CashFlow = () => {
  ...
  return <div className="grid gap-5"><div className="grid gap-4 md:grid-cols-3"><Card><p className="text-sm text-slate-500">Daily closing balance</p><p className="text-2xl font-bold">{money(income - expense)}</p></Card><Card><p className="text-sm text-slate-500">Monthly income</p><p className="text-2xl font-bold">{money(monthEntries.filter((e) => e.type === 'in').reduce((s, e) => s + e.amount, 0))}</p></Card><Card><p className="text-sm text-slate-500">Monthly expense</p><p className="text-2xl font-bold">{money(monthEntries.filter((e) => e.type === 'out').reduce((s, e) => s + e.amount, 0))}</p></Card></div><Card><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><h2 className="font-bold">Cash in / cash out</h2><Button className="w-full sm:w-auto" onClick={() => setOpen(true)}><Plus size={18} />Add entry</Button></div></Card><TableShell><table className="w-full"><thead><tr><th className={th}>Date</th><th className={th}>Type</th><th className={th}>Category</th><th className={th}>Amount</th><th className={th}>Notes</th></tr></thead><tbody>{(entries ?? []).map((e) => <tr key={e.id}><td className={td}>{e.date}</td><td className={td}><Badge tone={e.type === 'in' ? 'green' : 'red'}>{e.type === 'in' ? 'Cash in' : 'Cash out'}</Badge></td><td className={td}>{e.category}</td><td className={td}>{money(e.amount)}</td><td className={td}>{e.notes}</td></tr>)}</tbody></table></TableShell>{open && <CashModal onClose={() => setOpen(false)} onSaved={() => { notify('Cash entry saved'); setOpen(false); }} />}</div>;
};
