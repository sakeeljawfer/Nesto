...
export const Inventory = () => {
  ...
  return (
    <div className="grid gap-5">
      <Card><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div className="relative w-full min-w-0 flex-1 sm:max-w-md"><Search className="absolute left-3 top-2.5 text-slate-400" size={18} /><input className={`${inputClass} pl-10`} placeholder="Search products, brands, suppliers" value={query} onChange={(e) => setQuery(e.target.value)} /></div><Button className="w-full sm:w-auto" onClick={() => setEditing(blank())}><Plus size={18} />Add product</Button></div></Card>
      ...
      <Card><h2 className="mb-3 font-bold">Stock in / out history</h2><div className="grid gap-2">{(movements ?? []).slice(0, 8).map((m) => <div key={m.id} className="flex flex-col gap-2 rounded-md bg-slate-50 p-3 text-sm sm:flex-row sm:items-center sm:justify-between"><span className="min-w-0 break-words">{products?.find((p) => p.id === m.productId)?.name} - {m.reason}</span><Badge tone={m.type === 'in' ? 'green' : 'amber'}>{m.type} {m.quantity}</Badge></div>)}</div></Card>
