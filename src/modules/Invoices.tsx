...
const InvoiceModal = ({ data, onClose, onSaved }: { data: { invoices: Invoice[]; customers: Customer[]; products: Product[]; settings?: Settings }; onClose: () => void; onSaved: () => void }) => {
  ...
  return <Modal title="Create invoice" onClose={onClose}><div className="grid gap-4"><div className="grid gap-4 md:grid-cols-3"><Field label="Customer"><select className={inputClass} value={customerId} onChange={(e) => setCustomerId(e.target.value)}>{data.customers.map((c) => <option key={c.id} value={c.id}>{c.shopName}</option>)}</select></Field><Field label="Due date"><input className={inputClass} type="date" value={dueDate} onChange={(e) => setDueDate(e.target.value)} /></Field><Field label="Add product"><select className={inputClass} onChange={(e) => { addItem(e.target.value); e.currentTarget.value = ''; }} defaultValue=""><option value="" disabled>Select product</option>{data.products.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}</select></Field></div><TableShell><table className="w-full"><thead><tr><th className={th}>Item</th><th className={th}>Qty</th><th className={th}>Free</th><th className={th}>Price</th><th className={th}>Discount</th><th className={th}>Line</th></tr></thead><tbody>{items.map((item) => <tr key={item.lineId}><td className={td}>{item.productName}</td><td className={td}><input className={`${inputClass} w-24`} type="number" value={item.quantity} onChange={(e) => setItems((list) => list.map((x) => x.lineId === item.lineId ? { ...x, quantity: Number(e.target.value) } : x))} /></td><td className={td}><input className={`${inputClass} w-20`} type="number" value={item.freeItems} onChange={(e) => setItems((list) => list.map((x) => x.lineId === item.lineId ? { ...x, freeItems: Number(e.target.value) } : x))} /></td><td className={td}><input className={`${inputClass} w-28`} type="number" value={item.unitPrice} onChange={(e) => setItems((list) => list.map((x) => x.lineId === item.lineId ? { ...x, unitPrice: Number(e.target.value) } : x))} /></td><td className={td}><input className={`${inputClass} w-24`} type="number" value={item.discount} onChange={(e) => setItems((list) => list.map((x) => x.lineId === item.lineId ? { ...x, discount: Number(e.target.value) } : x))} /></td><td className={td}>{money(item.quantity * item.unitPrice - item.discount)}</td></tr>)}</tbody></table></TableShell><div className="grid gap-4 md:grid-cols-3"><Field label="Transport / delivery charge"><input className={inputClass} type="number" value={deliveryCharge} onChange={(e) => setDeliveryCharge(Number(e.target.value))} /></Field><Field label="Tax optional"><input className={inputClass} type="number" value={tax} onChange={(e) => setTax(Number(e.target.value))} /></Field><Field label="Cash / partial payment"><input className={inputClass} type="number" value={paidAmount} onChange={(e) => setPaidAmount(Number(e.target.value))} /></Field></div><div className="flex items-center justify-between rounded-lg bg-slate-50 p-4"><div><p className="text-sm text-slate-500">{nextNo}</p><p className="text-2xl font-bold">{money(totals.total)}</p></div><div className="flex gap-2"><Button variant="secondary" onClick={onClose}>Cancel</Button><Button onClick={save} disabled={!items.length || !customerId}>Save invoice</Button></div></div></div></Modal>;
};
