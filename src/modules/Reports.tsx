...
export const Reports = () => {
  ...
  return <div className="grid gap-5"><Card><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="font-bold">Reports</h2><p className="text-sm text-slate-500">Export reports as PDF or CSV for accounting.</p></div><div className="flex gap-2"><Button variant="secondary" onClick={() => downloadText('nesto-reports.csv', toCSV(reportRows), 'text/csv')}>Export CSV</Button><Button onClick={exportPdf}>Export PDF</Button></div></div></Card><TableShell><table className="w-full"><thead><tr><th className={th}>Report</th><th className={th}>Value</th></tr></thead><tbody>{reportRows.map((row) => <tr key={row.report}><td className={td}>{row.report}</td><td className={td}>{row.value}</td></tr>)}</tbody></table></TableShell></div>;
};
